...
%                 'BackgroundColor', dColorBg, ...
        ...
        function initHardware(this)
            ...
            % Default params struct for HIOs with common properties
            stDefault = struct();
            stDefault.clock = this.clock;
            stDefault.fhValidateDest = @this.validateDest;
            stDefault.lShowLabels = false;
            stDefault.lShowZero = false;
            stDefault.lShowRel = false;
            
            % Mono
            st = stDefault;
            st.config = configMono;
            st.cName = 'mono';
            st.lShowLabels = true;
            st.lShowZero = true;
            st.lShowRel = true;
            this.hioMono = HardwareIOPlus(st);
            
            % MaskX
            st = stDefault;
            st.cName = 'mask x';
            st.config = configMaskX;
            this.hioMaskX = HardwareIOPlus(st); 
            
            % MaskY
            st = stDefault;
            st.cName = 'mask y';
            st.config = configMaskY;
            this.hioMaskY = HardwareIOPlus(st);
            
            % MaskZ
            st = stDefault;
            st.cName = 'mask z';
            st.config = configMaskZ;
            this.hioMaskZ = HardwareIOPlus(st);
            
            % MaskT
            st = stDefault;
            st.cName = 'mask t';
            st.config = configMaskT;
            this.hioMaskT = HardwareIOPlus(st);
            
            % DetX
            st = stDefault;
            st.cName = 'det x';
            st.config = configDetX;
            this.hioDetX = HardwareIOPlus(st);
            
            % DetT
            st = stDefault;
            st.cName = 'det t';
            st.config = configDetT;
            this.hioDetT = HardwareIOPlus(st);
            
            % FilterY
            st = stDefault;
            st.cName = 'filter y';
            st.config = configFilterY;
            this.hioFilterY = HardwareIOPlus(st); 
